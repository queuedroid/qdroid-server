...
func RegisterRoutes(e *echo.Echo) {
	...
	api_v1.POST("/auth/send-verification-email", handlers.SendVerificationEmailHandler, middlewares.VerifyAuthMiddleware(middlewares.AuthMethodSession))
	api_v1.POST("/auth/verify-email", handlers.VerifyEmailHandler)
	api_v1.POST("/auth/resend-verification-email", handlers.ResendVerificationEmailHandler, middlewares.VerifyAuthMiddleware(middlewares.AuthMethodSession))
	api_v1.POST("/auth/forgot-password", handlers.ForgotPasswordHandler)
	api_v1.POST("/auth/reset-password", handlers.ResetPasswordHandler)
	...
	api_v1.GET("/sessions", handlers.GetSessionsHandler, middlewares.VerifyAuthMiddleware(middlewares.AuthMethodSession))
	api_v1.DELETE("/sessions/:session_id", handlers.DeleteSessionHandler, middlewares.VerifyAuthMiddleware(middlewares.AuthMethodSession))
	api_v1.DELETE("/sessions/delete-all", handlers.DeleteAllSessionsHandler, middlewares.VerifyAuthMiddleware(middlewares.AuthMethodSession))
	...
	api_v1.GET("/plans", handlers.GetPlansHandler)
